...
export function PuzzleHero() {
      ...
      >
        {active ? (
          ...
          <p className="text-xs uppercase tracking-label text-ash-300">
            Touchez une pièce — chaque pièce mène à une expertise
          </p>
        )}
      </motion.div>
    </AnimatePresence>
  ...
  // Mode 2D (mobile / sans WebGL) : hauteur automatique, label en dessous.
  if (mode === "2d") {
    return (
      <div className="relative flex w-full flex-col items-center">
        ...
        <div className="relative mt-4 flex justify-center px-4">{label}</div>
      </div>
    );
  }
